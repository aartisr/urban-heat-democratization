...
### Recent experience hardening

- The full-page map now opens unobstructed. Layer visibility lives behind a
  compact **Map layers** control; deeper source material is progressive rather
  than covering the map by default.
- The former before/after presentation was replaced with a clearly labelled
  planning influence preview. It communicates a priority-geometry shift and
  does not imply a measured temperature change or causal effect.
- City and scenario science cards now use a central dark-surface typography
  contract. Formula and disclosure treatments are part of the automated
  contrast check.
- Recommended scenario actions use a responsive single-column action card at
  constrained widths, preventing labels, evidence badges, and cost information
  from colliding.
- The shared app shell now resets desktop grid placement on tablet and mobile
  layouts, producing a readable full-width navigation header rather than a
  compressed vertical rail.
